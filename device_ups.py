...
def read_ups_data(shared_buffer_block):
    ...
    while True:
        try:
            ...
            shared_buffer_block.setValues(0, result.registers)  # 更新共享缓冲区中的UPS数据
            print(f"{datetime.now()} - UPS: 更新共享缓冲区中的UPS数据: {shared_buffer_block.getValues(0, 10)}")
